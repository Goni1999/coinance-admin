...
const TwoStepVerification = () => {
  ...
  useEffect(() => {
    const checkAuthStatus = async () => {
      ...
      try {
        const { data } = await axios.get("https://server.coinance.co/api/check-otp-status-admin", {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const sendOTP = async () => {
    try {
      ...
      const response = await axios.post(
        "https://server.coinance.co/api/send-otp-admin",
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
...
const verifyOtp = async (e: React.FormEvent) => {
  ...
  try {
    const response = await axios.post(
      "https://server.coinance.co/api/verify-otp-admin",
